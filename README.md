...
## The netbeasn project settings that are needed to run with the HDFS from the IDE:
...
## Working through trouble shooting the project settings.
...
After doing the above project built, but showed up the "no filesystem error". I noticed that Maven had not pulled in andy hdfs jars matching the jars in my share/hadoop/hdfs installation directory. So I took a guess that it needed only the top level hadoop-hdfs-2.7.0.jar and added this as a dependency to the POM and all was good :)
